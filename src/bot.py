...
logger.info(
    f"📢 Channel: {REMINDER_CHANNEL_ID}, Users: {len(MENTION_USER_IDS)}")
        ...
        if current_time == DAILY_REMINDER_TIME:
            try:
                ...
                if channel and isinstance(channel, discord.TextChannel):
                    mentions = " ".join(
                        [f"<@{user_id}>" for user_id in MENTION_USER_IDS])
                    ...
                    logger.info(
                        f"✅ Daily reminder sent to #{channel.name} at {current_time} {SYSTEM_TIMEZONE}")
                else:
                    logger.error(
                        f"❌ Channel not found or not a text channel. Channel ID: {REMINDER_CHANNEL_ID}")
            except Exception as e:
...
# Legacy command support (keeping the old prefix command for backward
# compatibility)
...
if __name__ == "__main__":
    ...
    if not DISCORD_TOKEN or DISCORD_TOKEN == "your_discord_token_here":
        logger.error(
            "❌ Error: DISCORD_TOKEN not set in environment variables or config_settings.py")
        logger.info(
            "💡 Please set your Discord bot token in the config_settings.py file or as an environment variable")
        exit(1)
