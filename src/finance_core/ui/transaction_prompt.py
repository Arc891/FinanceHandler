...
class DescriptionModal(Modal):
    def __init__(self, transaction_view, suggested_description: str):
        ...
        placeholder_text = suggested_description[:95] + "..." if len(
            suggested_description) > 95 else suggested_description
    ...
    async def _delete_after_delay(
            self, interaction: discord.Interaction, delay: int):
...
class TransactionView(View):
    def __init__(self, user_id: int, transaction: Dict[str, Any]):
        ...
        if ai_suggestion and ai_suggestion.get("category"):
            ...
            suggested_category, suggested_description = apply_categorization_rules(
                transaction)
        ...
        self.switch_type_button = Button(
            label=f"Switch to {'expense' if self.transaction_type == 'income' else 'income'}",
        ...
        self.cache_button = Button(
            label="Cache for Later",
            style=discord.ButtonStyle.secondary)
        self.cache_button.callback = self.cache_transaction
        self.add_item(self.cache_button)

    def _extract_suggested_description(
            self, transaction: Dict[str, Any]) -> str:
        ...
        if counterparty:
            ...
            cleaned_counterparty = counterparty.replace(
                "NL", "").replace("INGB", "").strip()
        ...
        if remittance and remittance[0]:
            ...
            if first_line and first_line not in description_parts:
                ...
                cleaned_remittance = first_line.replace(
                    "SEPA", "").replace("IBAN", "").strip()
    ...
    async def switch_type(self, interaction: discord.Interaction):
        ...
        if self.transaction_type == "income" and suggested_category in CATEGORY_OPTIONS[
                "income"]:
            self.selected_category = suggested_category
            self.suggested_description = suggested_description
    ...
    async def confirm_transaction(self, interaction: discord.Interaction):
        if not self.selected_category:
            ...
            asyncio.create_task(
                self._delete_response_after_delay(
                    interaction, 3))
            return

        # Use smart suggested description if available, otherwise fall back to
        # extracted description
        smart_suggestion = self.suggested_description
        if not smart_suggestion:
            smart_suggestion = self._extract_suggested_description(
                self.transaction)
    ...
    async def complete_transaction(
        ...
        if description:
            ...
            # Use smart suggested description if available, otherwise fall back
            # to extracted description
            if self.suggested_description:
                categorized_tx["description"] = self.suggested_description
                description_source = " + smart description"
            else:
                auto_desc = self._extract_suggested_description(
                    self.transaction)
        ...
        if '_cache_id' in tx:
            ...
            try:
                ...
                categorized_tx = {
                    k: v for k, v in categorized_tx.items() if k != '_cache_id'}
                queue_transaction_upload(
                    categorized_tx, self.transaction_type, self.user_id)
                upload_indicator = " 🔄📤"
                logger.info(f"Processed cached transaction {cache_id}")
            except Exception as e:
                logger.error(
                    f"❌ Failed to queue cached transaction for upload: {e}")
        ...
        if remaining:
            ...
            # All manual transactions processed - upload the manually
            # categorized ones
    ...
    async def cache_transaction(self, interaction: discord.Interaction):
        ...
        try:
            ...
            cache_id = cache_transaction(
                self.user_id, tx, self.transaction_type, auto_description)
            ...
            # Clean description without cache icon
            dummy_transaction["description"] = auto_description
            # Add cache_id for tracking
            dummy_transaction["cache_id"] = cache_id
    ...
    async def _delete_response_after_delay(
            self, interaction: discord.Interaction, delay: int):
        await asyncio.sleep(delay)
        try:
            await interaction.delete_original_response()
...
async def start_transaction_prompt(
    ...
    tx = remaining[0]
    embed = discord.Embed(
        title="🧾 Transaction to Categorize",
    ...
    embed.add_field(
        name="📈 Progress",
        value=f"{processed}/{total_transactions} completed",
        inline=True)
...
async def start_cached_transaction_prompt(
        interaction: discord.Interaction, user_id: int, cached_tx: Dict[str, Any]):
    ...
    embed = discord.Embed(
        title="🧾📦 Cached Transaction to Process",
    ...
    embed.add_field(
        name="📦 Cache Info",
        value=f"**ID:** {cached_tx['cache_id']}\n**Cached:** {cached_tx['timestamp'][:10]}",
        inline=True)
    ...
    # Create a special view for cached transactions (doesn't need session
    # management)
...
class CachedTransactionView(View):
    ...
    def __init__(self, user_id: int,
                 transaction: Dict[str, Any], cache_id: str):
    ...
    async def complete_cached_transaction(
            self, interaction: discord.Interaction, description: str):
        ...
        try:
            ...
            if description:
                ...
                # Use smart suggested description if available, otherwise fall
                # back to extracted description
                if self.suggested_description:
                    categorized_tx["description"] = self.suggested_description
                    description_source = " + smart description"
                else:
            ...
            # Store the reserved row in the transaction for the background
            # worker
            categorized_tx["_reserved_row"] = reserved_row

            # Queue transaction for replacement in Google Sheets (will replace
            # dummy entry)
            from finance_core.background_upload import queue_cached_replacement
            queue_cached_replacement(
                self.cache_id,
                categorized_tx,
                self.transaction_type,
                self.user_id)
            ...
            # Check if there are more cached transactions (after removing
            # current one)
            from finance_core.session_management import get_cached_transactions
            ...
            if remaining_cached:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to edit the original response: {e}")
